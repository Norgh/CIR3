...
function cleanStep() {
	scene.remove(scene.getObjectByName("BezierCurve"));
...
//Fonction pour tracer les étapes de construction de la courbe de Bezier, a le même resultat que casteljau
...
function DrawBezierCurve(pts){
	...
	let geo = new THREE.BufferGeometry().setFromPoints(points_tmp); //On récupère la courbe de Bezier puis on la trace
	let line = new THREE.Line(geo, materialA);
	line.name = "BezierCurve";
	line.width = 3;
